...
% Define directories
directories = {'3L', 'LL', 'L_G', 'LL_G'};
fault_types = {'3L', 'LL', 'LG', 'LLG'};
...
for dir_idx = 1:numel(directories)
    ...
    for bus_num = 0:6
        ...
        load(fullfile('imProc', current_dir, ['output_a_', fault_type, '_fault_bus', num2str(bus_num), '.mat']));
    
        % Load data from colorProc.mat
        load(fullfile('colorProc', current_dir, ['output_a_', fault_type, '_fault_bus', num2str(bus_num), '_colors.mat']));
    
        % Load data from grayEdge.mat
        load(fullfile('grayEdge', current_dir, ['output_a_', fault_type, '_fault_bus', num2str(bus_num), '_edges.mat']));
        ...
        clearvars -except directories fault_types current_dir fault_type dest
    end
